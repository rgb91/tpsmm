...
def make_animation(source_image, driving_video, inpainting_network, kp_detector, dense_motion_network, avd_network, device, mode = 'relative', desc=''):
    ...
    with torch.no_grad():
        ...
        for frame_idx in tqdm(range(driving.shape[2]), ascii=' >=', desc='Animation '+desc):
